...
def open_window(total):
    ...
    st.info("This is some information in a styled info box.")
...
def get_players_rate(players):
    ...
    ratio = t_1*tier1_w + t_2*tier2_w + t_3*tier3_w + t_4*tier4_w
...
def get_top50_players():
    ...
    top_players_list = stats_df.sort_values(by='TOTAL/G', ascending=False)['PLAYER_NAME'].head(50).tolist()
...
def get_general_rate(stats,pbp):
    ...
    elif c_lead > 4 and c_lead <= 10:
         leads = mark[2]

    elif c_lead > 10 and c_lead <= 16:
         leads = mark[3]
         
    elif c_lead > 16 and c_lead <= 22:
         leads = mark[4]
         
    elif c_lead > 22 and c_lead <= 30:
         leads = mark[5]
    ...
    grade = ot_mult*(difference*p_mult + score + leads)/3    
...
def get_individual_rate(team):
    ...
    h_rate = 0.85 #apply to all deffense stats added
    m3_rel = 1.5 #3 point relevance over fg
    #[0, 1, 2, 3], [worst, bad, good, incredible]
    

    #REAL VALUES
    FGP = team[1]*100; F3P = team[2]*100; F3M = team[3]
    AST = team[4]; TO = team[5]; OR = team[6]
    ...
    #HUSTLE
    if (OR + STL + BLK) >= (OR_ref[6] + STL_ref[6] + BLK_ref[6])*h_rate:
        hustle = MRK_ref[0] 

    elif ((OR + STL + BLK) < (OR_ref[6] + STL_ref[6] + BLK_ref[6])*h_rate) and ((OR + STL + BLK) >= (OR_ref[5] + STL_ref[5] + BLK_ref[5])*h_rate):
        hustle = MRK_ref[4]

    elif ((OR + STL + BLK) < (OR_ref[5] + STL_ref[5] + BLK_ref[5])*h_rate) and ((OR + STL + BLK) >= (OR_ref[4] + STL_ref[4] + BLK_ref[4])*h_rate):
        hustle = MRK_ref[4]

    elif ((OR + STL + BLK) < (OR_ref[4] + STL_ref[4] + BLK_ref[4])*h_rate) and ((OR + STL + BLK) >= (OR_ref[3] + STL_ref[3] + BLK_ref[3])*h_rate):
        hustle = MRK_ref[4]

    elif ((OR + STL + BLK) < (OR_ref[3] + STL_ref[3] + BLK_ref[3])*h_rate) and ((OR + STL + BLK) >= (OR_ref[2] + STL_ref[2] + BLK_ref[2])*h_rate):
        hustle = MRK_ref[4]

    elif ((OR + STL + BLK) < (OR_ref[2] + STL_ref[2] + BLK_ref[2])*h_rate) and ((OR + STL + BLK) >= (OR_ref[1] + STL_ref[1] + BLK_ref[1])*h_rate):
        hustle = MRK_ref[5]

    elif ((OR + STL + BLK) < (OR_ref[1] + STL_ref[1] + BLK_ref[1])*h_rate) and ((OR + STL + BLK) >= (OR_ref[0] + STL_ref[0] + BLK_ref[0])*h_rate):
        hustle = MRK_ref[6]
...
def get_teams_mult(matchup, standings):
    ...
    elif matchup[1] in standings[1] and matchup[2] in standings[1]:
        t_mult_2 = 1.07
